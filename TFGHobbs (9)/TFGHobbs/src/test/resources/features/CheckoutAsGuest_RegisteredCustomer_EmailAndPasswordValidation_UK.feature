@P2_ALL_DESKTOP_Checkout_registered_error_forms_UK
Feature:CheckoutAsGuest_RegisteredCustomer_EmailAndPasswordValidation_UK
  ...
  Scenario:  Select random size on PDP
    ...
    And user clicks add to bag button on PDP

  @GOTOBAGPAGE
  Scenario: Go to bag page
    When user clicks on view bag
    Then user is on basket page
    And the selected Product is added to bag
